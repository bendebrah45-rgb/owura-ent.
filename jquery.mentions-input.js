...
(function($, _, undefined) {
    ...
    var defaultSettings = {
        ...
        defaultValue: '',
        onCaret: false,
        noResultFoundMessage: __('no result found'),
        classes: {
        ...
        templates: {
            ...
            mentionItemSyntax: _.template('@[<%= id %>](<%= type %>:<%= id %>)'),
            mentionItemHighlight: _.template('<strong><span><%= id %></span></strong>')
        }
    };
    ...
    var utils = {
        ...
        highlightTerm: function(value, term) {
            ...
            return value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + utils.regexpEncode(term) + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<b>$1</b>");
        },
    ...
    var MentionsInput = function(settings) {
        ...
        function initTextarea() {
            ...
            elmInputBox.bind("paste", onInputPaste); //Bind the paste event to the text area
        ...
        function updateValues() {
            ...
            var uniq = _.uniq(mentionsCollection, function(x) {
                return x.id;
            });
            _.each(uniq, function(mention) {
                var textSyntax = settings.templates.mentionItemSyntax(mention);
                syntaxMessage = syntaxMessage.replace(new RegExp(utils.regexpEncode(mention.id), 'g'), textSyntax);
            });
            ...
            _.each(mentionsCollection, function(mention) {
                var formattedMention = _.extend({}, mention, {
                    value: utils.htmlEncode(mention.id)
                });
        ...
        function addMention(mention) {
            ...
            var regex = new RegExp("\\" + utils.regexpEncode(settings.triggerChar + currentDataQuery), "gi"),
                regexMatch;
            ...
            var startEndIndex = (start + mention.id).length + 1;
        ...
        */
        ...
        function onInputBoxBlur(e) {
            if (document.activeElement !== placeholderPrevActiveElement) {
                hideAutoComplete();
            }
        }
        ...
        function onInputPaste(e) {
            ...
            //Invoking the function doSearch ( Bind the function to this)
            var deletePasedText = function() {
                var textarea = elmInputBox[0];
                var new_value = elmInputBox.val().substr(0, textarea.selectionStart);
                console.log(new_value);
                elmInputBox.val(new_value);
            }
            doSearch(pastedText, deletePasedText);
        ...
        function onInputBoxKeyDown(e) {
            ...
            if (e.keyCode === KEY.BACKSPACE) {
                // remove last mention
                if (mentionsCollection.length) {
                    let textarea = elmInputBox[0];
                    if (textarea.selectionStart) {
                        let last_value = textarea.value;
                        let new_value = last_value.substring(0, textarea.selectionStart);
                        let start = new_value.lastIndexOf('{') - 1;
                        let mention_value = new_value.substring(start, textarea.selectionStart);
                        let found_mention = mentionsCollection.find(function(x) {
                            return x.id == mention_value;
                        });

                        if (found_mention) {
                            new_value = last_value.substring(0, start) + last_value.substring(textarea.selectionStart);
                            elmInputBox.val(new_value);
                            textarea.selectionStart = textarea.selectionEnd = start;
                            updateValues();
                        } else if (last_value.charAt(textarea.selectionStart - 1).match(/[{]+/) || checkCharacterIsInsideBraces(last_value, textarea.selectionStart - 1)) {
                            // want to only make the characters that don't get erased the only ones insider the braces 
                            e.preventDefault();
                        }

                        // get last mention
                        //let last_mention = mentionsCollection[ mentionsCollection.length-1 ];
                        // replace last mention value
                        //let new_value = last_value.replace(new RegExp(utils.regexpEncode(last_mention.value) + '$'), '');
                        //elmInputBox.val(new_value);
                        //updateValues();
                    }
                }
                ...
                if (textarea.selectionStart) {
                    let last_value = textarea.value;
                    if (last_value.charAt(textarea.selectionStart - 1).match(/[{]+/) || checkCharacterIsInsideBraces(last_value, textarea.selectionStart - 1)) {
                        // want to only make the characters that don't get erased the only ones insider the braces 
                        e.preventDefault();
                    }
                }
            ...
            function checkCharacterIsInsideBraces(query, index) {
                let numOpenedBeforeCharacter = (query.substring(0, index).match(new RegExp("\\{\{", "g")) || []).length;
                let numClosedBeforeCharacter = (query.substring(0, index).match(new RegExp("\\}\}", "g")) || []).length;
                let numOpenedAfterCharacter = (query.substring(index, query.length).match(new RegExp("\\{\{", "g")) || []).length;
                let numClosedAfterCharacter = (query.substring(index, query.length).match(new RegExp("\\}\}", "g")) || []).length;
                if (numOpenedBeforeCharacter == numClosedBeforeCharacter + 1 &&
                    numClosedAfterCharacter == numOpenedAfterCharacter + 1) {
                    return true;
                }
                return false;
            }
        ...
        function hideAutoComplete() {
            ...
            //elmInputBox.trigger( "autocomplete:hide" );
        }
        ...
        function populateDropdown(query, results) {
            ...
            if (!settings.allowRepeat) {
                ...
                var mentionValues = _.pluck(mentionsCollection, 'name');
            ...
            if (!results.length) { //If there are not elements hide the autocomplete list
                hideAutoComplete();
                //alert( settings.noResultFoundMessage );
                return;
            }
        ...
        function doSearch(query, callback = null) {
            //If the query is not null, undefined, empty and has the minimum chars
            // if (query && query.length && query.length >= settings.minChars) {
            if (query.length >= settings.minChars) {
                //Call the onDataRequest function and then call the populateDropDrown
                settings.onDataRequest.call(this, 'id', query, function(responseData) {
        ...
        function doList() {
            if (elmAutocompleteList.is(':visible')) {
                ...
                //Call the onDataRequest function and then call the populateDropDrown
                settings.onDataRequest.call(this, 'id', '', function(responseData) {
