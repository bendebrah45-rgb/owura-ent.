/*! selectize.js - v0.13.4 | https://github.com/selectize/selectize.js | Apache License (v2) */ ! function(root, factory) {
    ...
    function autoGrow($input) {
        function update(e, options) {
            ...
            options = options || {}, (e = e || window.event || {}).metaKey || e.altKey || !options.force && !1 === $input.data("grow") || (options = $input.val(), e.type && "keydown" === e.type.toLowerCase() && (printable = 48 <= (keyCode = e.keyCode) && keyCode <= 57 || 65 <= keyCode && keyCode <= 90 || 96 <= keyCode && keyCode <= 111 || 186 <= keyCode && keyCode <= 222 || 32 === keyCode, 46 === keyCode || 8 === keyCode ? (selection = getSelection($input[0])).length ? options = options.substring(0, selection.start) + options.substring(selection.start + selection.length) : 8 === keyCode && selection.start ? options = options.substring(0, selection.start - 1) + options.substring(selection.start + 1) : 46 === keyCode && void 0 !== selection.start && (options = options.substring(0, selection.start) + options.substring(selection.start + 1)) : printable && (keyCode = e.shiftKey, selection = String.fromCharCode(e.keyCode), options += selection = keyCode ? selection.toUpperCase() : selection.toLowerCase())), printable = $input.attr("placeholder"), e = $input, (selection = ((keyCode = options = !options && printable ? printable : options) ? (Selectize.$testInput || (Selectize.$testInput = $("<span />").css({
                ...
                height: 0,
                overflow: "hidden"
            }).append(Selectize.$testInput).appendTo("body")), Selectize.$testInput.text(keyCode), transferStyles(e, Selectize.$testInput, ["letterSpacing", "fontSize", "fontFamily", "fontWeight", "textTransform"]), Selectize.$testInput.width()) : 0) + 4) !== currentWidth && (currentWidth = selection, $input.width(selection), $input.triggerHandler("resize")))
        }
    ...
    return MicroEvent.mixin(Selectize), void 0 !== MicroPlugin ? MicroPlugin.mixin(Selectize) : (MicroPlugin = "Dependency MicroPlugin is missing", options = (options = {
        ...
        createItem: function(input, triggerDropdown) {
            ...
            var fn, called, setup = "function" == typeof self.settings.create ? this.settings.create : function(input) {
                    ...
                    if (self.settings.formatValueToKey && "function" == typeof self.settings.formatValueToKey && (null == (input = self.settings.formatValueToKey.apply(this, [input])) || "object" == typeof input || "function" == typeof input)) throw new Error('Selectize "formatValueToKey" setting must be a function that returns a value other than object or function.');
        ...
        deleteSelection: function(e) {
            var i, n, values, option_select, $option_select, $tail, direction = e && 8 === e.keyCode ? -1 : 1,
        ...
        var original, self = this,
            getScrollbarWidth = (options = $.extend({
                ...
                var index, $optgroup;
                return !this.isOpen || 37 !== e.keyCode && 39 !== e.keyCode ? original.apply(this, arguments) : (self.ignoreHover = !0, index = ($optgroup = this.$activeOption.closest("[data-group]")).find("[data-selectable]").index(this.$activeOption), void(($optgroup = (e = ($optgroup = 37 === e.keyCode ? $optgroup.prev("[data-group]") : $optgroup.next("[data-group]")).find("[data-selectable]")).eq(Math.min(e.length - 1, index))).length && this.setActiveOption($optgroup)))
            }), function() {
